...
class TaskApi(Resource):
    ...
    def get(self, project_id: int, task_id: str):
        ...
        current_app.config["rpc"].call("project", "get_or_404", project_id=project_id)
        args = self.get_parser.parse_args(strict=False)
        task = Task.query.filter_by(task_id=task_id).first()
        if args.get("exec"):
            return current_app.config["rpc"].call("project", "unsecret", value=task.to_json(), project_id=project_id)
        return task.to_json()
