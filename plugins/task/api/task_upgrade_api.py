...
class TaskUpgradeApi(Resource):
    ...
    def get(self, project_id):
        ...
        secrets = current_app.config["rpc"].call("project", "get_hidden_secrets", project_id=project["id"])
        project_secrets = {}
        if args['name'] == 'post_processor':
            ...
            secrets = current_app.config["rpc"].call("project", "set_secrets",
                                                     project_id=project["id"],
                                                     secrets=project_secrets)
        else:
            return {"message": "go away", "code": 400}, 400
        current_app.config["rpc"].call("project", "set_hidden_secrets",
                                       project_id=project["id"],
                                       secrets=secrets)
